...
param(
    ...
    [string]$SourceImageDir = "E:\Stable Diffusion\Inputs\df\lib\female\dani",  # Path to source images
    
    [Parameter(Mandatory=$false)]
    [string]$QwenModelPath = "Qwen/Qwen2.5-VL-7B-Instruct",  # Path to Qwen2.5-VL model (default: HuggingFace model ID)
    
    [Parameter(Mandatory=$false)]
    [string]$TargetDir = "E:\Stable Diffusion\TrainingDataSet\dani",  # Target directory for processed dataset
    
    [Parameter(Mandatory=$false)]
    [string]$TriggerWord = "dani",
...
Write-Host "=== Preparing Dani Dataset ===" -ForegroundColor Cyan
...
if (-not $SkipCopy) {
    ...
    foreach ($Ext in $ImageExtensions) {
        # Recursively search for images in source directory and all subfolders
    ...
    if ($SkippedCount -gt 0) {
        Write-Host "  Skipped $SkippedCount duplicate images" -ForegroundColor Yellow
    }
    Write-Host ""
} else {
